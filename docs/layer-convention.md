...
현재 모든 규칙에는 실제 검사 대상이 있으므로 `allowEmptyShould(true)` 를 쓰지 않는다. 패키지명을 잘못 적거나 소스가 누락돼 검사 대상이 0개가 되면 테스트가 실패해야 한다. 규칙을 추가하거나 패키지를 리네임할 때는 반드시 위반 클래스를 임시로 심어 테스트가 FAIL 하는 것을 확인한다.
