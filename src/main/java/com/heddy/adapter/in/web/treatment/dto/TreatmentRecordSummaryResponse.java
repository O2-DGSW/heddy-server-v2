...
public record TreatmentRecordSummaryResponse(
        ...
        @Schema(description = "최신 분석 상태. 분석을 요청한 적 없는 기록은 null 이다. "
                + "\"분석 완료\" 는 SUCCEEDED 만 해당한다 — 사진이 바뀌면 STALE 로 전이되므로 "
                + "결과가 남아 있다는 것과 완료라는 것은 같지 않다. 허용값은 이 열거형이 정한다",
                nullable = true)
        @JsonProperty("analysis_status") AnalysisJobStatus analysisStatus,
