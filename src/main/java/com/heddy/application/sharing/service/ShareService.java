...
/**
 * 공유 생성·목록 조회. 생성 때는 대상 소유권을 검증하고(#31 컨벤션) 토큰을 발급해 해시로만
 * 저장한다 — 원문은 201 응답의 share_url 로 딱 한 번 세상에 나온다.
 *
 * <p>소유권 검증은 포트 질의에 소유자 조건을 실어 보내는 방식이다. 남의 기록은 없는 기록과 같은
 * 404 이고, 질의 횟수도 같아야 존재 여부가 새지 않는다.
 */
@Service
