...
/**
 * API 명세 18절 오류 코드의 공통 계약.
 *
 * <p>공통 코드(18.1절)는 {@link CommonErrorCode}가 갖고, 도메인 코드는 각 도메인 패키지에서
 * {@code com.heddy.<도메인>.error.<도메인>ErrorCode} enum 으로 이 인터페이스를 구현해 추가한다.
 * 도메인 코드를 여기에 미리 몰아 정의하지 않는다 — 명세가 확정된 도메인만 자기 코드를 갖는다.
 *
 * <p>추가 규칙은 {@code docs/layer-convention.md} 4절 "에러 응답" 참고.
 */
public interface ErrorCode {
    ...
    /** 이 코드가 내려갈 HTTP 상태. API 명세 2.4절 사용 기준을 따른다. */
    HttpStatus status();
