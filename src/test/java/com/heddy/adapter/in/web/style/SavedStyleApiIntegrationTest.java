...
class SavedStyleApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void updatesAndClearsMemoOnlyForTheOwner() throws Exception {
        ...
        mockMvc.perform(patch("/me/saved-styles/" + savedStyleId)
                        .with(authentication(userAuthentication(USER_ID)))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void returnsEveryCandidateNewestFirstAndRejectsAnEmptyPatch() throws Exception {
        UUID first = saveCandidate(USER_ID, insertHairstyle("후보 하나", null), null);
        ...
        mockMvc.perform(get("/me/saved-styles")
                ...
                .andExpect(jsonPath("$.data.items[1].saved_style_id")
                        .value(first.toString()))
                .andExpect(jsonPath("$.data.page").doesNotExist());

        mockMvc.perform(patch("/me/saved-styles/" + second)
                        .with(authentication(userAuthentication(USER_ID)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
